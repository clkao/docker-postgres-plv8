...
Docker images for running [plv8](https://github.com/plv8/plv8) 1.4 and 1.5 on Postgres 9 (9.4 and 9.5). Based on the [official Postgres image](http://registry.hub.docker.com/_/postgres/).
...
- `9.5-1.5`, `latest` ([9.5-1.5/Dockerfile](https://github.com/clkao/docker-postgres-plv8/blob/master/9.5-1.5/Dockerfile))
...
```sh
$ docker run --rm --name postgres -it clkao/postgres-plv8:9.5-1.5
$ docker run --rm --link postgres:postgres -it postgres-plv8:9.5-1.5 bash -c 'psql -U postgres -h $POSTGRES_PORT_5432_TCP_ADDR -t -c "CREATE EXTENSION plv8; SELECT extversion FROM pg_extension WHERE extname = ''plv8'';"'
```
...
```yml
postgres:
  image: clkao/postgres-plv8:9.5-1.5
```
...
[docker-layers-image]: https://img.shields.io/imagelayers/layers/clkao/postgres-plv8/latest.svg?style=flat-square
[docker-pulls-image]: https://img.shields.io/docker/pulls/clkao/postgres-plv8.svg?style=flat-square
[docker-size-image]: https://img.shields.io/imagelayers/image-size/clkao/postgres-plv8/latest.svg?style=flat-square
